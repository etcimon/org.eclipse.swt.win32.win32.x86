        ...
        if (colors.length is 1) {
            ...
            if (vertical) {
                if (onBottom) {
                    ...
                    for (int i = cast(int) percents.length-1; i >= 0; i--) {
...
void drawChevron(GC gc) {
    ...
    int count;
    if (single) {
        count = selectedIndex is -1 ? cast(int) items.length : cast(int) items.length - 1;
    } else {
        int showCount = 0;
        while (showCount < priority.length && items[priority[showCount]].showing) {
            showCount++;
        }
        count = cast(int) items.length - showCount;
    }
...
void initAccessible() {
    ...
    accessible.addAccessibleControlListener(new class() AccessibleControlAdapter {
        ...
        public void getChildAtPoint(AccessibleControlEvent e) {
            ...
            if (childID is ACC.CHILDID_NONE) {
                if (showChevron && chevronRect.contains(testPoint)) {
                    childID = cast(int) items.length + CHEVRON_CHILD_ID;
                } else if (showMin && minRect.contains(testPoint)) {
                    childID = cast(int) items.length + MINIMIZE_CHILD_ID;
                } else if (showMax && maxRect.contains(testPoint)) {
                    childID = cast(int) items.length + MAXIMIZE_CHILD_ID;
                } else {
        ...
        public void getChildCount(AccessibleControlEvent e) {
            e.detail = cast(int) items.length + EXTRA_CHILD_ID_COUNT;
        }
        ...
        public void getChildren(AccessibleControlEvent e) {
            int childIdCount = cast(int) items.length + EXTRA_CHILD_ID_COUNT;
...
bool setItemLocation() {
    ...
    if (single) {
        ...
        int x = 0;
        int defaultX = getDisplay().getBounds().width + 10; // off screen
        firstIndex = cast(int) items.length - 1;
        for (int i = 0; i < items.length; i++) {
            CTabItem item = items[i];
            if (!item.showing) {
...
public void setSelectionBackground(Color[] colors, int[] percents, bool vertical) {
    ...
    if (colors !is null) {
        ...
        if(percents.length is colors.length - 2) {
            ...
            colorsLength = cast(int) colors.length - 1;
        } else {
            colorsLength = cast(int) colors.length;
        }
        if (getDisplay().getDepth() < 15) {
            // Don't use gradients on low color displays
