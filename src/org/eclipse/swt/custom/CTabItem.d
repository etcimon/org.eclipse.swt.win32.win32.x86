...
void drawHighlight(GC gc, int rightEdge) {
    ...
    if(gradients is null)
        return;
    int gradientsSize = cast(int) gradients.length;
    ...
    int d = parent.tabHeight - cast(int) parent.topCurveHighlightEnd.length /2;
...
int preferredWidth(GC gc, bool isSelected, bool minimum) {
    ...
    if (minimum) {
        ...
        if (text !is null && toString32(text).length > minChars) {
            if (useEllipses()) {
                int end = minChars < ELLIPSIS.length + 1 ? minChars : minChars - cast(int) ELLIPSIS.length;
