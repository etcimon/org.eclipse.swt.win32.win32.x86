...
void addLineIndex(int start, int length) {
    int size = cast(int) lines.length;
...
int[][] addLineIndex(int start, int length, int[][] linesArray, int count) {
    int size = cast(int) linesArray.length;
...
void indexLines(){
    int start = 0;
    lineCount_ = 0;
    int textLength = cast(int) textStore.length;
    int i;
...
void insert(int position, String text) {
    ...
    int change = cast(int) text.length;
    ...
    int startLineLength = cast(int) getPhysicalLine(startLine).length;
    ...
    int numNewLines = cast(int) newLines.length - 1;
...
public int getCharCount() {
    ...
    return cast(int) (textStore.length - length);
}
/**
...
public void replaceTextRange(int start, int replaceLength, String newText){
    ...
    event.newCharCount = cast(int) newText.length;
    sendTextEvent(event);
...
void delete_(int position, int length_, int numLines) {
    ...
    int numOldLines = cast(int) oldLines.length - 1;
