...
package:
    ...
    void print() {
        ...
        if (printOptions.printLineNumbers) {
            ...
            if (lineLabels !is null) {
                for (int i = startLine; i < Math.min(count, cast(int) lineLabels.length); i++) {
    ...
    void printDecoration(int page, bool header, TextLayout layout) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (segmentIndex is -1 ) {
                ...
                printDecorationSegment(segment, i, page, header, layout);
                lastSegmentIndex = segmentIndex + cast(int) StyledTextPrintOptions.SEPARATOR.length;
            }
        }
    }
    /**
    ...
    void printDecorationSegment(String segment, int alignment, int page, bool header, TextLayout layout) {
        ...
        if (pageIndex !is -1 ) {
            int pageTagLength = cast(int) StyledTextPrintOptions.PAGE_TAG.length;
...
//          }
    ...
    int getColorIndex(Color color, int defaultIndex) {
        ...
        if (index is -1) {
            index = cast(int) colorTable.length;
            colorTable ~= color;
        }
        return index;
    }
    ...
    int getFontIndex(Font font) {
        ...
        if (index is -1) {
            index = cast(int) fontTable.length;
            fontTable ~= font;
        }
        return index;
    }
...
//                         (!osName.startsWith(WinNT) || majorVersion > 4);
    ...
    void writeHeader() {
        ...
        if (cpg.startsWith("cp") || cpg.startsWith("ms")) {
            cpg = cpg.substring(2, cast(int) cpg.length());
    ...
    public override void writeLine(String line, int lineOffset) {
        ...
        if (event !is null) {
            ...
            ranges = renderer.getRanges(lineOffset, cast(int) line.length);
            styles = renderer.getStyleRanges(lineOffset, cast(int) line.length, false);
        }
    ...
    public override void writeLineDelimiter(String lineDelimiter) {
        ...
        write(lineDelimiter, 0, cast(int) lineDelimiter.length);
        write("\\par ");
    }
    /**
    ...
    void writeStyledLine(String line, int lineOffset, int ranges[], StyleRange[] styles, Color lineBackground, int indent, int alignment, bool justify) {
        int lineLength = cast(int) line.length;
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            if (ranges !is null) {
                ...
                end = start + cast(int) style.length;
            }
...
void doBackspace() {
    ...
    if (selection.x !is selection.y) {
        ...
        if (caretOffset is lineOffset) {
            ...
            event.start = lineOffset + cast(int)content.getLine(lineIndex - 1).length;
...
void doDelete() {
    ...
    if (selection.x !is selection.y) {
        ...
        int lineLength = cast(int) content.getLine(line).length;
...
void doLineEnd() {
    ...
    if (wordWrap) {
        ...
        int lineLength = cast(int) content.getLine(caretLine).length;
    ...
    if (isSingleLine()) {
        ...
        if (wordWrap) {
            ...
            if (index is -1 && lineIndex > 0) {
				bottomOffset = content.getOffsetAtLine(lineIndex - 1) +  cast(int) content.getLine(lineIndex - 1).length;
            } else {
            ...
            bottomOffset = content.getOffsetAtLine(lineIndex) + cast(int) content.getLine(lineIndex).length;
        }
...
void doSelectionCursorPrevious() {
    ...
    if (offsetInLine > 0) {
        ...
        caretOffset = lineOffset + cast(int) content.getLine(caretLine).length;
        showCaret();
    }
}
/**
...
public int getBaseline(int offset) {
    ...
    int lineOffset = content.getOffsetAtLine(lineIndex);
    TextLayout layout = renderer.getTextLayout(lineIndex);
    int lineInParagraph = layout.getLineIndex(Math.min(offset - lineOffset, cast(int) layout.getText().length));
...
Rectangle getBoundsAtOffset(int offset) {
    ...
    if (offset is caretOffset) {
        int lineEnd = lineOffset + cast(int) line.length;
    ...
    if (trailing[0] !is 0) {
        ...
        if (offsetInLine + trailing[0] is lineStart) {
            ...
            while (offset > 0 && Character.isDigit(line.dcharAt(offset)))
                offset = cast(int) line.offsetBefore(offset);
...
int [] getBidiSegments(int lineOffset, String line) {
    ...
    if (event is null || event.segments is null || event.segments.length is 0) {
        ...
        int segmentCount = cast(int) event.segments.length;
...
int [] getBidiSegmentsCompatibility(String line, int lineOffset) {
    ...
    for (int i = k; i < styles.length; i++) {
        ...
        int styleLineEnd = Math.max(style.start + cast(int) style.length - lineOffset, styleLineStart);
        styleLineEnd = Math.min (styleLineEnd, cast(int) line.length );
        if (i > 0 && count > 1 &&
...
public Rectangle getTextBounds(int start, int end) {
    ...
    for (int i = lineStart; i <= lineEnd; i++) {
        ...
        int length = cast(int) layout.getText().length;
        if (length > 0) {
            if (i is lineStart) {
                if (i is lineEnd) {
...
int getWordNext (int offset, int movement) {
    ...
    if (offset >= getCharCount()) {
        ...
        lineOffset = content.getOffsetAtLine(lineIndex);
        lineText = content.getLine(lineIndex);
        int lineLength = cast(int) lineText.length;
...
int getWordPrevious(int offset, int movement) {
    ...
    if (offset <= 0) {
        ...
        if (offset is lineOffset) {
            ...
            newOffset = nextLineOffset + cast(int) nextLineText.length;
        } else {
            TextLayout layout = renderer.getTextLayout(lineIndex);
...
void internalRedrawRange(int start, int length) {
    ...
    int[] offsets = layout.getLineOffsets();
    int startIndex = layout.getLineIndex(Math.min(start, cast(int) layout.getText().length));
    ...
    if (startLine is endLine) {
        int endIndex = layout.getLineIndex(Math.min(end, cast(int) layout.getText().length));
...
void modifyContent(Event event, bool updateCaret) {
    ...
    if (event.doit) {
        ...
        if (isListening(ExtendedModify)) {
            ...
            styledTextEvent.end = event.start + cast(int) event.text.length;
        ...
        if (updateCaret) {
			...
			setSelection(event.start +  cast(int) event.text.length, 0, true);
            showCaret();
        }
...
void sendModifyEvent(Event event) {
    ...
    if (event.text.length is 0) {
        ...
        if (event.start is event.end) {
            accessible.textChanged(ACC.TEXT_INSERT, event.start, cast(int) event.text.length);
        } else {
            accessible.textChanged(ACC.TEXT_DELETE, event.start, event.end - event.start);
            accessible.textChanged(ACC.TEXT_INSERT, event.start, cast(int) event.text.length);
        }
...
public void setStyleRange(StyleRange range) {
    ...
    if (range !is null) {
        if (range.isUnstyled()) {
            setStyleRanges(range.start, cast(int) range.length, null, null, false);
        } else {
...
int getPreviousCharOffset(String F = __FILE__, uint L = __LINE__)(int lineIndex, int offsetInLine) {
    ...
    if(offsetInLine < 0 || offsetInLine > line.length) {
        getDwtLogger().warn(F, L, Format("Clamped UTF-8 offset:\noffsetInLine = {}, cast(int) line.length = {}, line = {}", offsetInLine, cast(int) line.length, line));
        return offsetInLine - 1;
    }
    return cast(int) line.offsetBefore(offsetInLine);
}
}
