...
public void javaToNative (Object object, TransferData transferData){
    ...
    int startHTML = cast(int) HEADER.length;
    int startFragment = startHTML + cast(int) PREFIX.length;
    int endFragment = startFragment + cchMultiByte - 1;
    int endHTML = endFragment + cast(int) SUFFIX.length;

    StringBuffer buffer = new StringBuffer(HEADER);
    int maxLength = cast(int) NUMBER.length;
    //startHTML
...
public Object nativeToJava(TransferData transferData){
    ...
    try {
        ...
        try {
			...
			OS.MultiByteToWideChar (OS.CP_UTF8, 0, lpMultiByteStr, -1, lpWideCharStr.ptr, cast(int) lpWideCharStr.length);
            String string = WCHARzToStr(lpWideCharStr.ptr);
            int fragmentStart = 0, fragmentEnd = 0;
            int start = string.indexOf(StartFragment) + cast(int) StartFragment.length;
            int end = start + 1;
            ...
            start = string.indexOf(EndFragment) + cast(int) EndFragment.length;
            end = start + 1;
            while (end < string.length) {
                String s = string.substring(start, end);
                try {
            ...
            if (fragmentEnd <= fragmentStart || fragmentEnd > OS.strlen(lpMultiByteStr)) return null;
				cchWideChar = OS.MultiByteToWideChar (OS.CP_UTF8, 0, lpMultiByteStr+fragmentStart, fragmentEnd - fragmentStart, lpWideCharStr.ptr, cast(int) lpWideCharStr.length);
            if (cchWideChar is 0) return null;
