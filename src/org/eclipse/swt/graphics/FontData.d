...
public this(String string) {
    ...
    if (platform ==/*eq*/ "WINDOWS" && version2 ==/*eq*/ "1") {  //$NON-NLS-1$//$NON-NLS-2$
		...
		int len = Math.min(OS.LF_FACESIZE - 1, cast(int) wname.length);
...
public String getLocale () {
    ...
    int length_ = cast(int) result.length;
    if (length_ > 0) {
...
public void setLocale(String locale) {
    ...
    if (locale !is null) {
        char sep = '_';
        int length_ = cast(int) locale.length;
...
public void setName(String name) {
	...
	int len = Math.min( cast(int) OS.LF_FACESIZE - 1, cast(int) wname.length);
