...
/++
 ...
 +/
    ...
    static const UTF16shift WMARK_SIZE = cast(int) WSTR_LTR_MARK.length;
...
void breakRun(StyleItem run) {
    ...
    auto hHeap = OS.GetProcessHeap();
	run.psla = cast(SCRIPT_LOGATTR*)OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, cast(int) (SCRIPT_LOGATTR.sizeof * wchars.length));
    if (run.psla is null) SWT.error(SWT.ERROR_NO_HANDLES);
	OS.ScriptBreak(wchars.ptr, cast(int) wchars.length, &run.analysis, run.psla);
}
...
out {
    ...
    for (int i=0; i<allRuns.length - 1; i++) {
        ...
        if (run.UTF8length.internalValue is 1) {
				char ch = segmentsText.charAt( cast(int) run.UTF8start.internalValue );
            assert(ch == segmentsText.dcharAt(run.UTF8start));
            switch (ch) {
                case '\t': {
                    run.tab = true;
                    if (tabs is null) break;
					int tabsLength = cast(int) tabs.length, j;
                ...
                case '\r': {
					...
					if (next.UTF8length.internalValue !is 0 && segmentsText.charAt( cast(int) next.UTF8start.internalValue ) is '\n') {
                ...
                if (wstart is 0 && i !is lineStart && !run.tab) {
                    if (logAttr.fSoftBreak && !logAttr.fWhiteSpace) {
                        ...
                        if (properties.langid is langID || langID is OS.LANG_NEUTRAL || properties.langid is OS.LANG_NEUTRAL) {
                            ...
                            //OS.MoveMemory(logAttr, pRun.psla + cast(int) ((pRun.length - 1) * SCRIPT_LOGATTR.sizeof), SCRIPT_LOGATTR.sizeof);
...
public void draw (GC gc, int x, int y, int i_selectionStart, int i_selectionEnd, Color selectionForeground, Color selectionBackground, int flags) {
    ...
    int wlength = cast(int) wtext.length;
        ...
        if (style.underline) {
            ...
            switch (style.underlineStyle) {
                ...
                case SWT.UNDERLINE_ERROR: {
					...
					Gdip.Graphics_DrawLines(cast(Gdip.Graphics)graphics, pen, cast(Gdip.Point*)points.ptr, cast(int) (points.length / 2));
                    Gdip.Pen_delete(pen);
        ...
        if (style.underline) {
            ...
            switch (style.underlineStyle) {
                ...
                case SWT.UNDERLINE_ERROR: {
					...
					OS.Polyline(graphics, cast(POINT*)points.ptr,  cast(int) (points.length / 2));
                    int length_ = cast(int) points.length;
    ...
    for (int i = 0; i < allRuns.length - 1; i++) {
        ...
        if (run.UTF8start <= start && start < runEnd) {
            ...
            if (run.style !is null && run.style.metrics !is null) {
				...
				cx = cast(int) (metrics.width * (getUTF16index(start) - getUTF16index(run.UTF8start)));
            } else if (!run.tab) {
        ...
        if (run.UTF8start <= end && end < runEnd) {
            ...
            if (run.style !is null && run.style.metrics !is null) {
                GlyphMetrics metrics = run.style.metrics;
				cx = cast(int) (metrics.width * (getUTF16index(end) - getUTF16index(run.UTF8start) + 1));
            } else if (!run.tab) {
...
public Point getLocation (int i_offset, bool trailing) {
	...
	line = cast(int) Math.min(line, runs.length - 1);
    if (offset is length) {
        return new Point(getLineIndent(line) + lineWidth[line], lineY[line]);
    }
    int low = -1;
    int high = cast(int) allRuns.length;
    while (high - low > 1) {
        ...
        if (run.UTF8start > offset) {
            ...
            if (run.style !is null && run.style.metrics !is null) {
                GlyphMetrics metrics = run.style.metrics;
				width = cast(int) (metrics.width * (getUTF16index(offset) - getUTF16index(run.UTF8start) + trailing));
            } else if (run.tab) {
...
int _getOffset(int i_offset, int movement, bool forward) {
    ...
    UTF8index length = text.endIndex();
    if (!(0 <= offset.internalValue && offset <= length)) SWT.error(SWT.ERROR_INVALID_RANGE);
	if (forward && offset is length) return cast(int) length.internalValue;
    if (!forward && offset.internalValue is 0) return 0;
    int step = forward ? 1 : -1;
	if ((movement & SWT.MOVEMENT_CHAR) !is 0) return cast(int) (offset + text.toUTF8shift(offset, step)).internalValue;
    length = segmentsText.endIndex();
    offset = translateOffset(offset);
    SCRIPT_LOGATTR* logAttr;
    SCRIPT_PROPERTIES* properties;
    int i = forward ? 0 : cast(int) allRuns.length - 1;
    ...
    return forward ? cast(int) text.length : 0;
}
...
public int getOffset (int x, int y, int[] trailing) {
    ...
    int line;
    int lineCount = cast(int) runs.length;
    ...
    int low = -1;
    int high = cast(int) lineRuns.length;
    while (high - low > 1) {
        int index = ((high + low) / 2);
        ...
        if (run.x > x) {
            ...
            if (run.style !is null && run.style.metrics !is null) {
                ...
                if (metrics.width > 0) {
                    ...
                    if (trailing !is null) {
						trailing[0] =  cast(int) ((xRun % metrics.width < metrics.width / 2) ? 0 : segmentsText.UTF8strideAt(res).internalValue);
                    }
            ...
            if (run.tab) {
				...
				if (trailing !is null) trailing[0] = cast(int) (x < (run.x + run.width / 2) ? 0 : segmentsText.UTF8strideAt(res).internalValue);
                return untranslateOffset(res);
            }
            UTF16shift cChars = getUTF16length(run); // make it wchar
            ...
            if (trailing !is null)
				trailing[0] = cast(int) (addUTF16shift(res, piTrailing) - res).internalValue;
            return untranslateOffset(res);
        }
    }
...
public int[] getRanges () {
    ...
    for (int i=0; i<stylesCount - 1; i++) {
        if (styles[i].style !is null) {
			result[count++] = cast(int) styles[i].UTF8start.internalValue;
			result[count++] = cast(int) getUTF8index(cast(UTF16index)(getUTF16index(styles[i + 1].UTF8start) - 1)).internalValue;
        }
    }
...
void getSegmentsText( out String resUtf8, out String16 resUtf16 ) {
    ...
    int nSegments = cast(int) segments.length;
    if (nSegments <= 1) {
        resUtf8 = text;
        resUtf16 = wtext;
        buildIndexTables();
    ...
    int wlength_ = cast(int) wtext.length;
    if (length_ is 0) {
        resUtf8 = text;
        resUtf16 = wtext;
        buildIndexTables();
    ...
    }
    {
        ...
        while (charCount < length_) {
            if (segmentCount < nSegments && charCount is segments[segmentCount].internalValue) {
				int start = cast(int) (charCount + (segmentCount*MARK_SIZE.internalValue));
    ...
    // now for the wide chars
    {
        ...
        while (charCount < wlength_) {
            if (segmentCount < nSegments && charCount is wsegments[segmentCount]) {
				int start = cast(int) (charCount + (segmentCount*WMARK_SIZE));
...
StyleItem[] itemize () {
    ...
    int length = cast(int) segmentsText.length;
    ...
    auto hHeap = OS.GetProcessHeap();
	auto pItems = cast(SCRIPT_ITEM*)OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, cast(int) (MAX_ITEM * SCRIPT_ITEM.sizeof + 1));
    if (pItems is null) SWT.error(SWT.ERROR_NO_HANDLES);
    int pcItems;
    String16 wchars = segmentsWText;
	OS.ScriptItemize(wchars.ptr, cast(int) wchars.length, MAX_ITEM, &scriptControl, &scriptState, pItems, &pcItems);
...
StyleItem[] reorder (StyleItem[] runs, bool terminate) {
    int length_ = cast(int) runs.length;
...
bool shape (HDC hdc, StyleItem run, String16 wchars, int[] glyphCount, int maxGlyphs, SCRIPT_PROPERTIES* sp) {
    ...
    if (useCMAPcheck) {
		...
		if (OS.ScriptGetCMap(hdc, run.psc, wchars.ptr, cast(int) wchars.length, 0, glyphs.ptr) !is OS.S_OK) {
            if (run.psc !is null) {
	...
	auto hr = OS.ScriptShape(hdc, run.psc, wchars.ptr, cast(int) wchars.length, maxGlyphs, &run.analysis, run.glyphs, run.clusters, run.visAttrs, glyphCount.ptr);
...
void shape (HDC hdc, StyleItem run) {
    ...
    int maxGlyphs = cast(int) (wchars.length * 3 / 2) + 16;
    auto hHeap = OS.GetProcessHeap();
    ...
    if (!shapeSucceed) {
		...
		if (OS.ScriptStringAnalyse(metaFileDc, wchars.ptr, cast(int) wchars.length, 0, -1, flags, 0, null, null, null, null, null, ssa) is OS.S_OK) {
        ...
        if (!shapeSucceed) {
            if (mLangFontLink2 !is null) {
                ...
                /* GetStrCodePages() */
				OS.VtblCall(4, mLangFontLink2, cast(int)wchars.ptr, cast(int) wchars.length, 0, cast(int)&dwCodePages, cast(int)&cchCodePages);
                /* MapFont() */
    ...
    if (!shapeSucceed) {
        ...
        */
		OS.ScriptShape(hdc, run.psc, wchars.ptr, cast(int) wchars.length, maxGlyphs, &run.analysis, run.glyphs, run.clusters, run.visAttrs, buffer.ptr);
...
int untranslateOffset(UTF8index offset) {
	if (segments is null) return cast(int) offset.internalValue;
    int nSegments = cast(int) segments.length;
	if (nSegments <= 1) return cast(int) offset.internalValue;
    int length = cast(int) text.length;
	if (length is 0) return cast(int) offset.internalValue;
    if (nSegments is 2) {
		if (segments[0].internalValue is 0 && segments[1].internalValue is length) return cast(int) offset.internalValue;
    }
    for (int i = 0; i < nSegments && offset > segments[i]; i++) {
        offset-=MARK_SIZE;
    }
	return cast(int) offset.internalValue;
}
}
