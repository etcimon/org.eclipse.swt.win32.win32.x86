...
void setData(in byte[] data) {
	setLength(cast(int) data.length);
...
static PngChunk readNextFromStream(LEDataInputStream stream) {
    try {
		...
		int result = cast(int) stream.read(headerBytes, 0, cast(int) headerLength);
		...
		result = cast(int) stream.read(chunk, cast(int) 0, cast(int) chunkLength);
