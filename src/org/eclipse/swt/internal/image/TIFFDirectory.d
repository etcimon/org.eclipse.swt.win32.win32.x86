...
void decodePixels(ImageData image)  {
    ...
    int destIndex = 0;
    int length = cast(int) stripOffsets.length;
...
int formatStrips(int rowByteSize, int nbrRows, byte[] data, int maxStripByteSize, int offsetPostIFD, int extraBytes, int[][] strips) {
    ...
    */
    ...
    if (rowByteSize > maxStripByteSize) {
        ...
        n = cast(int) data.length / rowByteSize;
        nbrRowsPerStrip = 1;
    } else {
        int nbr = cast(int) (data.length + maxStripByteSize - 1) / maxStripByteSize;
    ...
    */
    ...
    int mod = cast(int) data.length % stripByteSize;
...
int[] formatColorMap(RGB[] rgbs) {
    ...
    */
    ...
    int offsetGreen = cast(int) rgbs.length;
    int offsetBlue = cast(int) rgbs.length * 2;
...
void write(int photometricInterpretation)  {
    ...
    int cnt = cast(int) stripOffsets.length;
    if (cnt > 1) {
	...
	if (isColorMap) writeEntry(TAG_ColorMap, TYPE_SHORT, cast(int) colorMap.length, colorMapOffset);
