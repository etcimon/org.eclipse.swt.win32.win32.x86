...
void seek(int pos) {
    ...
    if (current > next) {
        ...
        while (n > 0) {
			...
			int cnt = cast(int) inputStream.read(buffers[index], cast(int) offset, cast(int) Math.min(n, CHUNK_SIZE - offset));
            n -= cnt;
            next += cnt;
            index++;
            offset = 0;
...
void read(byte b[]) {
    ...
    if (nMissing > 0) {
        ...
        while (nMissing > 0) {
            ...
            if (buffers[index] is null) buffers[index] = new byte[CHUNK_SIZE];
			int cnt = cast(int) inputStream.read(buffers[index], offset, Math.min(nMissing, CHUNK_SIZE - offset));
            System.arraycopy(buffers[index], offset, b, destNext, cnt);
            nMissing -= cnt;
            next += cnt;
            destNext += cnt;
