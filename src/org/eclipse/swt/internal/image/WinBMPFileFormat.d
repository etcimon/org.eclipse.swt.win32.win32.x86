...
void decompressData(byte[] src, byte[] dest, int stride, int cmp) {
    if (cmp is 1) { // BMP_RLE8_COMPRESSION
		if (decompressRLE8Data(src, cast(int) src.length, stride, dest, cast(int) dest.length) <= 0)
            SWT.error(SWT.ERROR_INVALID_IMAGE);
        return;
    }
    if (cmp is 2) { // BMP_RLE4_COMPRESSION
		if (decompressRLE4Data(src, cast(int) src.length, stride, dest, cast(int) dest.length) <= 0)
            SWT.error(SWT.ERROR_INVALID_IMAGE);
        return;
    }
    SWT.error(SWT.ERROR_INVALID_IMAGE);
...
static byte[] paletteToBytes(PaletteData pal) {
    int n = pal.colors is null ? 0 : (pal.colors.length < 256 ? cast(int) pal.colors.length : 256);
...
override void unloadIntoByteStream(ImageLoader loader) {
    ...
    if ((image.depth is 16) || (image.depth is 24) || (image.depth is 32)) {
        ...
        if (pal.isDirect)
            SWT.error(SWT.ERROR_INVALID_IMAGE);
        numCols = cast(int) pal.colors.length;
    ...
    // Calculate file size
    fileHeader[1] = fileHeader[4] + cast(int) data.length;
    ...
    try {
		...
		outputStream.writeInt(cast(int) data.length);
