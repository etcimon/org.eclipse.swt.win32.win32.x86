...
int iconSize(ImageData i) {
    ...
    int paletteSize = i.palette.colors !is null ? cast(int) i.palette.colors.length * 4 : 0;
...
bool isValidIcon(ImageData i) {
    switch (i.depth) {
        ...
        case 8:
            ...
            int size = cast(int) i.palette.colors.length;
...
ImageData loadIcon(int[] iconHeader) {
	...
	bitInvertData(maskData, 0, cast(int) maskData.length);
...
void unloadIcon(ImageData icon) {
    ...
    try {
        ...
        outputStream.writeInt(0);
        outputStream.writeInt(0);
        outputStream.writeInt(icon.palette.colors !is null ? cast(int) icon.palette.colors.length : 0);
        outputStream.writeInt(0);
    } catch (IOException e) {
        SWT.error(SWT.ERROR_IO, e);
    }
...
void unloadIconHeader(ImageData i) {
    ...
    try {
        ...
        outputStream.writeShort(i.palette.colors !is null ? cast(int) i.palette.colors.length : 0);
