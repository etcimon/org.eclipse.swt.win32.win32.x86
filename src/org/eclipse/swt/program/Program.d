...
static String assocQueryString (int assocStr, StringT key, bool expand) {
    ...
    uint[1] pcchOut;
    pcchOut[0] = cast(int) pszOut.length;
...
public static String [] getExtensions () {
    ...
    uint [1] lpcName; lpcName[0] = cast(int) lpName.length;
    FILETIME ft;
    ...
    while (OS.RegEnumKeyEx ( cast(void*)OS.HKEY_CLASSES_ROOT, dwIndex, lpName.ptr, lpcName.ptr, null, null, null, &ft) !is OS.ERROR_NO_MORE_ITEMS) {
        ...
        lpcName [0] = cast(int) lpName.length;
        ...
        if (length_ !is 0) {
            ...
            if (OS.RegQueryValueEx (phkResult [0], null, null, null, cast(ubyte*)lpData.ptr, lpcbData.ptr) is 0) {
                if (!OS.IsWinCE && expand) {
					...
					length_ = cast(int) Math.max (0, lpData.length - 1);
...
public bool execute (String fileName) {
    ...
    while (index < ARGUMENTS.length) {
        ...
        if (i !is -1) {
			...
			suffix = command.substring (cast(int) (i + ARGUMENTS [index].length) , cast(int) command.length );
            break;
        }
        index++;
    }
...
public ImageData getImageData () {
    ...
    if (index !is -1) {
		...
		String iconIndex = iconName.substring (index + 1, cast(int) iconName.length ).trim ();
        try {
