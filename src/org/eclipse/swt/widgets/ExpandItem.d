...
private void drawChevron (HDC hDC, RECT* rect) {
	...
	OS.Polyline (hDC, cast(POINT*)polyline1.ptr, cast(int) (polyline1.length / 2));
	OS.Polyline (hDC, cast(POINT*)polyline2.ptr, cast(int) (polyline2.length / 2));
    if (hover) {
        ...
        int [] points1 = [
                ...
                rect.left, rect.top,
                rect.right, rect.top];
		OS.Polyline (hDC, cast(POINT*)points1.ptr, cast(int) (points1.length / 2));
        OS.SelectObject (hDC, darkGrayPen);
        int [] points2 = [
                rect.right, rect.top,
                rect.right, rect.bottom,
                rect.left, rect.bottom];
		OS.Polyline (hDC, cast(POINT*)points2.ptr, cast(int) (points2.length / 2));
...
void drawItem (GC gc, HTHEME hTheme, RECT* clipRect, bool drawFocus) {
    ...
    if (text.length > 0) {
        ...
        if (hTheme !is null) {
			OS.DrawThemeText (hTheme, hDC, OS.EBP_NORMALGROUPHEAD, 0, buffer.ptr, cast(int) buffer.length, OS.DT_VCENTER | OS.DT_SINGLELINE, 0, &rect);
        } else {
            int oldBkMode = OS.SetBkMode (hDC, OS.TRANSPARENT);
			OS.DrawText (hDC, buffer.ptr, cast(int) buffer.length, &rect, OS.DT_VCENTER | OS.DT_SINGLELINE);
    ...
    if (expanded) {
        if (!parent.isAppThemed ()) {
			...
			OS.Polyline (hDC, cast(POINT*) points.ptr, cast(int) (points.length / 2));
            OS.SelectObject (hDC, oldPen);
            OS.DeleteObject (pen);
        }
    }
...
int getPreferredWidth (HTHEME hTheme, HDC hDC) {
    ...
    if (text.length > 0) {
        ...
        if (hTheme !is null) {
			OS.GetThemeTextExtent (hTheme, hDC, OS.EBP_NORMALGROUPHEAD, 0, buffer.ptr, cast(int) buffer.length, OS.DT_SINGLELINE, null, &rect);
        } else {
			OS.DrawText (hDC, buffer.ptr, cast(int) buffer.length, &rect, OS.DT_CALCRECT);
        }
